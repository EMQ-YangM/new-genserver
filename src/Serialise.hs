...
delayChannel
  :: (MonadSTM m, MonadTimer m) => DiffTime -> Channel m a -> Channel m a
...
mvarsAsChannel bufferRead bufferWrite = Channel { send, recv }
 where
...
createConnectedChannels = do
  ...
  return (mvarsAsChannel bufferB bufferA, mvarsAsChannel bufferA bufferB)
...
convertCborDecoderLBS cborDecode liftST = go []

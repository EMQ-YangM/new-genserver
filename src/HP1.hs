...
type Peer :: [(Type -> Type) -> Type] -> (Type -> Type) -> Type -> Type
data Peer ms m a where
  RGetSelfQueueVal :: Peer ms m (Sum ms n)
  RGetSelfNodeID :: Peer ms m NodeID
  --------------------------------------------------------------------
  RCall :: SCall t req resp :< ms => NodeID -> (Call req resp -> t) -> req -> Peer ms m resp
  RCast :: SCast t msg :< ms => NodeID -> (Cast msg -> t) -> msg -> Peer ms m ()
  RGet :: SGet t resp :< ms => NodeID -> (Get resp -> t) -> Peer ms m resp
  RCallAll ::
    SCall t req resp :< ms =>
    (Call req resp -> t) ->
    req ->
    Peer ms m [(NodeID, resp)]
  RCastAll :: SCast t msg :< ms => (Cast msg -> t) -> msg -> Peer ms m ()
  RGetAll :: SGet t resp :< ms => (Get resp -> t) -> Peer ms m [(NodeID, resp)]
